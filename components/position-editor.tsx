...
import { positionChanges, type PositionChange } from "@/lib/position-save";
import { Modal } from "@/components/modal";

export function PositionEditor({ match, onSave, onClose }: { match:MatchData; onSave:(changes:PositionChange[])=>Promise<void>; onClose:()=>void }) {
  const [original] = useState(match.players);
  const [players,setPlayers] = useState(match.players);
  const changes = positionChanges(original, players);
  const [busy,setBusy] = useState(false);
  const [error,setError] = useState("");
  async function save() {
    setBusy(true); setError("");
    try { await onSave(changes); } catch (failure) { setError(failure instanceof Error ? failure.message : "Could not save positions. Your changes are still here; please retry."); }
  ...
  return <Modal labelId="positions-title" className="editor-dialog" onClose={()=>{if(!busy) onClose();}}>
    ...
    {error&&<p role="alert" className="notice">{error}</p>}<div className="dialog-actions"><button type="submit" className="button primary" disabled={busy || !changes.length}>{busy?"Saving…":"Save positions"}</button><button type="button" className="button secondary" onClick={onClose} disabled={busy}>Cancel</button></div></form>
  </Modal>;
}
