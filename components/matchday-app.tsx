...
import { includeCurrentMatch, seasonComparison, seasonLabel } from "@/lib/season";
...
export function MatchdayApp() {
  ...
  const [mvpPreferences, setMvpPreferences] = useState<Record<string, string[]>>({});
  ...
  const mvpKey = `${scope}:${match.id}:${phase}`;
  const mvpIds = selectMvpIds(rated.map(({player,score})=>({id:player.id,name:player.name,score})), mvpPreferences[mvpKey]);
  const chosenMvps = mvpIds.map(id=>rated.find(row=>row.player.id===id)!);
  const hasMvpTies = chosenMvps.some(row=>rated.filter(other=>other.score===row.score).length>1);
  const seasonAverages = season.unavailable || season.loading ? {} : includeCurrentMatch(season.averages, match, store.ratings.ft);
  ...
  async function openPoster(featuredIds = mvpIds) {
    ...
    try {
      ...
      const canvas = buildMatchprintCanvas({ match, phase, players: eligible.map((player) => ({ player, rating: actualRatings[player.id], halfTimeRating: store.ratings.ht[player.id], seasonAverage: seasonAverages[player.id] })), seasonUnavailable: season.unavailable, featuredPlayerIds: featuredIds });
  ...
  return <>
    ...
    <main id="top" className="app-main">
      ...
      </section><aside className="summary-panel" aria-labelledby="summary-title"><div className="section-heading"><div><span className="eyebrow">Your verdict</span><h2 id="summary-title">My match</h2></div><span className="phase-label">{phase.toUpperCase()}</span></div><div className="team-average"><div><span>Team average</span><small>{rated.length ? `From ${rated.length} rated players` : "Rate a player to begin"}</small></div><RatingBadge value={average} large /></div><div className="community-line"><span>Community average</span><strong>{formatRating(crowdAverage)}</strong></div><div className="leaders"><div className="section-heading"><h3>MVPs</h3><span className="muted">Your highest ratings</span></div><ol>{chosenMvps.map(({ player, score }, index) => <li key={player.id}><span className="leader-rank">{index + 1}</span><div className="leader-copy"><strong>{player.name}</strong><small>{contributionLabel(contributions[player.id]) || player.roleLabel}</small></div><RatingBadge value={score} /></li>)}</ol>{!rated.length && <p className="empty-copy">Your top performers will appear here.</p>}</div><button className="button primary full-width" onClick={() => void openPoster()} disabled={!rated.length || posterBusy}>{posterBusy ? "Creating your image…" : "Preview Matchprint"}</button><p className="export-note">{remaining ? `${remaining} players still to rate. Partial sheets are labelled.` : "Your match sheet is complete."}</p><div className="save-status" role="status"><span className={store.pending ? "status-dot pending" : "status-dot"} /><span>{store.status}</span></div>{store.conflict ? <button className="text-button" onClick={() => store.retry(true)}>Keep this device’s edits and sync</button> : user && (store.pending > 0 || store.status.includes("unavailable")) && <button className="text-button" onClick={() => store.retry()}>Retry account sync</button>}{!user && <p className="guest-note">Guest ratings stay on this device. <button className="text-button" onClick={() => setAuthOpen(true)}>Sign in to sync</button></p>}</aside></div>
      {phase === "ft" && rated.length > 0 && <section className="season-section" aria-labelledby="season-title"><div className="section-heading"><div><span className="eyebrow">{seasonLabel(match.kickoff)}</span><h2 id="season-title">Against your season</h2></div><span className="muted">Full-time season averages</span></div><div className="season-list">{rated.map(({ player, score }) => <div key={player.id} className="season-player"><strong>{player.name}</strong><RatingBadge value={score} /><div><span>{season.unavailable ? "History unavailable" : seasonComparison(score, seasonAverages[player.id])}</span><small>{seasonAverages[player.id] ? `${seasonAverages[player.id].matches} rated ${seasonAverages[player.id].matches === 1 ? "match" : "matches"}` : ""}</small></div></div>)}</div><p className="method-note">Your personal FT ratings through this match, including this match when finished. Season: July–June.{Object.values(seasonAverages).some((item) => item.includesConverted) ? " Includes earlier /5 scores converted to /10." : ""}</p></section>}
    ...
    {poster && <Modal labelId="poster-title" className="poster-dialog" onClose={() => setPoster(null)}><div className="dialog-heading"><div><span className="eyebrow">Ready to share</span><h2 id="poster-title">Your Matchprint</h2></div><button className="icon-button" onClick={() => setPoster(null)} aria-label="Close Matchprint">×</button></div>
      {hasMvpTies && <fieldset className="mvp-picker"><legend>Choose your MVPs</legend><p className="muted">Pick and order players with tied scores. Higher scores keep their place.</p><div>{chosenMvps.map(({score},slot)=><label key={slot}>MVP {slot+1}<select aria-label={`MVP ${slot+1}`} disabled={posterBusy} value={mvpIds[slot]} onChange={event=>{const next=replaceMvp(mvpIds,slot,event.target.value);setMvpPreferences(previous=>({...previous,[mvpKey]:next}));void openPoster(next);}}>{rated.filter(row=>row.score===score).map(({player})=><option key={player.id} value={player.id}>{player.name} · {formatRating(score)}</option>)}</select></label>)}</div></fieldset>}
      {posterBusy && <p role="status">Updating your preview…</p>}
      {/* This PNG is created on the device and contains no external image assets. */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img className="poster-image" src={poster.url} alt={`${phase.toUpperCase()} ratings for ${match.home.name} versus ${match.away.name}`} /><div className="dialog-actions"><button className="button primary" disabled={posterBusy} onClick={downloadPoster}>Download PNG</button><button className="button secondary" disabled={posterBusy} onClick={() => void sharePoster()}>Share image</button></div><p className="muted">Created on your device. No upload is required.</p></Modal>}
