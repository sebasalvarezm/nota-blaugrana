...
const {positionChanges,parsePositionChanges,positionSaveError}=load("lib/position-save");
const savePlayerId="00000000-0000-4000-8000-000000000001";
const saveMatchId="00000000-0000-4000-8000-000000000101";
const initialPosition={id:savePlayerId,role:"FB",roleLabel:"Full-back",x:12.5,y:64.3};
const finalPosition={...initialPosition,roleLabel:"Right-back",x:82,y:72};
const saveChanges=positionChanges([initialPosition],[finalPosition]);
assert.equal(saveChanges.length,1);
assert.equal(saveChanges[0].expected.pitch_x,12.5);
assert.deepEqual(positionChanges([initialPosition],[initialPosition]),[]);
assert.deepEqual(parsePositionChanges(saveChanges),saveChanges);
assert.equal(parsePositionChanges([...saveChanges,...saveChanges]),null);
...
let dbError=null,dbRows=[savedPosition],authEmail="owner@example.test",calls=0;
modules.set(path.join(root,"lib/supabase/server.ts"),{getServerSupabase:service=>service?{rpc:()=>{calls++;return{abortSignal:async()=>({error:dbError,data:dbRows})};}}:{auth:{getUser:async()=>({data:{user:{email:authEmail}},error:null})}}});
process.env.ADMIN_EMAILS="owner@example.test";
const {POST:savePositionRequest}=load("app/api/matches/positions/route");
const saveRequest=(body={matchId:saveMatchId,positions:saveChanges},token="fixture")=>new Request("http://fixture.invalid/api/matches/positions",{method:"POST",headers:{"Content-Type":"application/json",...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify(body)});
assert.equal((await savePositionRequest(saveRequest(undefined,""))).status,401);
authEmail="visitor@example.test";
assert.equal((await savePositionRequest(saveRequest())).status,403);
assert.equal(calls,0);
authEmail="owner@example.test";
assert.equal((await savePositionRequest(saveRequest({matchId:saveMatchId,positions:[]}))).status,400);
let response=await savePositionRequest(saveRequest());
assert.equal(response.status,200);
assert.equal((await response.json()).positions[0].pitch_x,82);
...
console.log("Position patch validation, owner authorization, actionable failures and verified save responses passed.");